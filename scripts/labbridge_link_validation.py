...
def main() -> int:
    ...
    assert "--allow-ollama-fallback" in cli, "runtime fallback must remain explicit opt-in"
    assert "--watch" in cli, "local live link watcher is missing"
    print("PASS: BetterBoard discovery, Engineering Lab record ownership and OpenPenguin advisory boundaries are preserved")
    return 0
