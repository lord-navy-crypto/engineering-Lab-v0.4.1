...
def main() -> int:
    ...
    import physical_lab_utube_visualization as v

    geom = v.reference_geometry(bend_points=121)
    ...
    profile = v.effective_potential_profile(260.0, points=241)
    ...
    state = v.capacity_decomposition(3.0, 260.0, nq=40)
    ...
    phase = v.threshold_phase_map([3.0], [max(1.0, nc - 5.0), 0.5 * (nc + ng), ng, ng + 10.0], nq=32, near_threshold_band_rpm=1.0)
    ...
    require("not experimental observations" in v.BOUNDARY, "visualization evidence boundary weakened")
