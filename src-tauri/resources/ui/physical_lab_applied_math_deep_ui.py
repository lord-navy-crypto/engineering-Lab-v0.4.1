...
def _linear_algebra_tab(st: Any, source: dict[str, Any], profile: str) -> None:
    ...
    method = st.radio(
        ...
        ["PCA / SVD", "Conditioning", "Regularized inverse", "TSVD / GCV / L-curve"],
        horizontal=True,
        key=f"pl_deep_linear_method_{profile}",
    )
...
def render_applied_math_deep(st: Any, profile: str) -> None:
    ...
    task = st.radio(
        "Deep analysis task",
        ["Linear Algebra & Inverse Problems", "Completed Sweep Feedback + Handoff"],
        horizontal=True,
        key=f"pl_deep_task_{profile}",
    )

    if task == "Completed Sweep Feedback + Handoff":
