...
def _run_options(project_path: Path) -> list[dict[str, Any]]:
    ...
    for job_id, entry in (doc.get("results") or {}).items():
        if not isinstance(entry, dict):
            continue
        rows.append({
            "key": f"project::{job_id}",
            "label": f"Project · {job_id}",
            "kind": "project",
            "job_id": str(job_id),
        })
    try:
        ...
        for job in list_sweep_jobs(limit=100):
            ...
            for i, point in enumerate(full.get("points") or []):
                if not isinstance(point, dict) or point.get("status") != "succeeded" or not isinstance(point.get("result"), dict):
                    continue
                rows.append({
                    "key": f"sweep::{job['id']}::{i}",
                    "label": f"Sweep · {job.get('adapter')} · {job['id']} · point {i}",
                    "kind": "sweep",
                    "job_id": str(job["id"]),
                    "point_index": int(i),
                })
...
def _snapshot(option: dict[str, Any], project_path: Path) -> dict[str, Any]:
    if option["kind"] == "project":
        return project_run_snapshot(project_path, option["job_id"])
    return sweep_point_snapshot(option["job_id"], int(option["point_index"]))
...
def render_run_comparison(st: Any, profile: str) -> None:
    ...
    st.caption(
        "Compare parameters, scalar metrics, numerical sanity, explicit uncertainty, software environment and provenance. "
        "Comparability labels are evidence warnings, not scientific significance tests."
    )
    ...
    key_b = c2.selectbox("Run B", keys, index=default_b, format_func=lambda k: by_key[k]["label"], key=f"pl_compare_b_{profile}")
    if key_a == key_b:
    ...
    try:
        ...
        st.warning(f"Could not load selected run snapshots: {exc}")
        return
    ...
    tab_summary, tab_params, tab_metrics, tab_quality, tab_stale = st.tabs([
        "Summary", "Parameters", "Metrics", "Quality & UQ", "Staleness / Environment"
    ])
    ...
    with tab_summary:
        ...
        st.caption(comparison["boundary"])
    ...
    with tab_quality:
        ...
        with q1:
            st.markdown("#### Run A")
            st.json(comparison["sanity"]["a"])
            uq = comparison["uncertainty"]["a"]
            if uq: st.json(uq)
            else: st.caption("No explicit uncertainty object.")
        with q2:
            st.markdown("#### Run B")
            st.json(comparison["sanity"]["b"])
            uq = comparison["uncertainty"]["b"]
            if uq: st.json(uq)
            else: st.caption("No explicit uncertainty object.")
    ...
    with tab_stale:
        ...
        st.markdown("#### Current environment reference")
        st.json(current_env)

    with st.expander("Comparison fingerprint / raw evidence", expanded=False):
        st.json({
