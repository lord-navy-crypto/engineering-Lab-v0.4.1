...
def _robust_design(st: Any, profile: str) -> None:
    ...
    if st.button("Evaluate tolerance envelope & Pareto", type="primary", key=f"pl_ut_rob_go_{profile}"):
        try:
            frame = robust_design_space(
                volumes_ml=_numbers(volumes_text),
                rin_values_m=_numbers(rin_text),
                a_values_m=_numbers(a_text),
                volume_tolerance_ml=vtol,
                rin_tolerance_m=rtol,
                a_tolerance_m=atol,
    ...
    if isinstance(frame, pd.DataFrame) and not frame.empty:
        ...
        m1.metric("Nominal designs", len(frame))
        m2.metric("Pareto candidates", int(frame["pareto"].sum()))
        m3.metric("Best separation floor", f"{frame['separation_floor_rpm'].max():.3f} rpm")
        st.dataframe(frame.head(300), hide_index=True, width="stretch")
        fig = px.scatter(
            frame,
            x="worst_abs_target_error_rpm",
            y="threshold_span_rpm",
            color="separation_floor_rpm",
            symbol="pareto",
            hover_data=["V_mL", "R_in_m", "a_m", "nominal_threshold_rpm", "corner_count"],
            title="Robust-design Pareto view · target error vs tolerance sensitivity",
        )
...
def _adaptive_planner(st: Any, profile: str) -> None:
    ...
    text = st.text_area(
        "Observed classifications · one `rpm,below` or `rpm,above` per line",
        value="",
        placeholder="240,below\n260,above",
        key=f"pl_ut_adapt_obs_{profile}",
    )
    try:
        ...
        a, b, c = st.columns(3)
        a.metric("Model n_g", f"{prediction:.3f} rpm")
        b.metric("Planner state", result["status"])
        c.metric("Bracket width", "—" if result["empirical_bracket_width_rpm"] is None else f"{result['empirical_bracket_width_rpm']:.3f} rpm")
        st.json(result)
...
def _verification(st: Any, profile: str) -> None:
    ...
    try:
        matrix = verification_requirements(
            target_threshold_rpm=target,
            threshold_tolerance_rpm=tol,
            minimum_separation_rpm=sep,
            maximum_numerical_change_rpm=num,
        )
    except Exception as exc:
        st.warning(f"Verification matrix unavailable: {exc}")
        return
    ...
    if not active:
        st.info("Open an Engineering Lab Project to register these templates into Requirements & Verification.")
        return
        ...
        try:
            for row in matrix.to_dict("records"):
                record = requirements.register_requirement(
                    ...
                    verification_activity=str(row["verification_activity"]),
                    success_criteria=str(row["success_criteria"]),
                    references=[],
                    source_document="U-Tube Advanced Physics & Engineering",
                    source_locator="Verification Matrix template",
                    rationale=str(row["rationale"]),
                    level="subsystem",
                    intended_use="U-tube model development and project evidence review",
                )
...
def render_utube_robust_engineering(st: Any, profile: str) -> None:
    ...
    with tab_robust:
        _robust_design(st, profile)
    with tab_adaptive:
        _adaptive_planner(st, profile)
    with tab_verify:
        _verification(st, profile)
    st.caption(BOUNDARY)
