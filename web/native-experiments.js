...
const NATIVE_PARAMETER_SCHEMAS = Object.freeze({
  'numerical-methods':[
    {name:'method',label:'Method',type:'select',value:'range_reduced',options:['range_reduced','direct','fixed_terms']},
    ...
    {name:'referencePrecisionDigits',label:'mpmath decimal precision',type:'number',value:80,min:20,max:200,step:5},
    {name:'toleranceMultiplier',label:'Tolerance multiplier × machine epsilon',type:'number',value:8,min:.1,max:1000,step:.1},
    {name:'maxTerms',label:'Maximum Taylor terms',type:'number',value:120,min:1,max:500,step:1},
    {name:'xMin',label:'x minimum (radians)',type:'number',value:-8,min:-100,max:100,step:.1},
    {name:'xMax',label:'x maximum (radians)',type:'number',value:8,min:-100,max:100,step:.1},
    {name:'points',label:'Number of scan points',type:'number',value:401,min:21,max:5001,step:20},
    {name:'singleX',label:'Single x value (radians)',type:'number',value:1.5,min:-100,max:100,step:.01},
    {name:'convergenceTerms',label:'Terms in convergence study',type:'number',value:60,min:2,max:300,step:1}
  ],
  'ising-monte-carlo':[
    ...
    {name:'temperature',label:'Temperature T (J/kB)',type:'number',value:2.269,min:.01,max:20,step:.01},
    ...
    {name:'diagnosticCycles',label:'Diagnostic update cycles',type:'number',value:1000,min:10,max:100000,step:10},
    {name:'recordEvery',label:'Record every n update cycles',type:'number',value:10,min:1,max:1000,step:1},
    ...
    {name:'useNotebookMesh',label:'Use original notebook temperature mesh',type:'checkbox',value:false},
  ...
  'random-walk-monte-carlo':[
    ...
    {name:'scanVariable',label:'Scan variable',type:'select',value:'n_steps',options:['n_steps','dimension','n_walkers','step_size']},
    ...
    {name:'presetJson',label:'Preset JSON',type:'text',value:'{}'}
  ],
  'nonlinear-chaos':[
    ...
    {name:'damping',label:'Damping β (s⁻¹)',type:'number',value:.05,min:0,max:20,step:.01},
    {name:'kapitzaTotalPeriods',label:'Total periods',type:'number',value:240,min:10,max:5000,step:10},
    {name:'kapitzaDiscardedPeriods',label:'Discarded periods',type:'number',value:160,min:0,max:4000,step:10},
    {name:'duration',label:'Duration (s)',type:'number',value:40,min:.1,max:1000,step:.5},
    {name:'dt',label:'Timestep dt (s)',type:'number',value:.005,min:.00001,max:1,step:.001},
    ...
    {name:'flipMaxTime',label:'Maximum time',type:'number',value:60,min:.1,max:1000,step:1},
  ...
  'oscillation-integration':[
    ...
    {name:'scanPoints',label:'Scan points',type:'number',value:9,min:3,max:101,step:2},
    {name:'resonanceGamma',label:'Resonance damping γ',type:'number',value:.1,min:0,max:100,step:.01},
    {name:'resonanceForce',label:'Driving force F₀',type:'number',value:1,min:0,max:1000,step:.01},
    {name:'frequencyScanPoints',label:'Frequency scan points',type:'number',value:61,min:5,max:501,step:2},
    {name:'omegaRatioMin',label:'Minimum Ω/ω₀',type:'number',value:.2,min:.01,max:10,step:.01},
    {name:'omegaRatioMax',label:'Maximum Ω/ω₀',type:'number',value:2,min:.02,max:10,step:.01},
    {name:'maxInitialAngle',label:'Maximum initial angle (rad)',type:'number',value:2.5,min:.01,max:3.13,step:.01},
    {name:'amplitudeScanPoints',label:'Amplitude scan points',type:'number',value:31,min:5,max:301,step:2},
  ...
  'radia-magnet-studio':[
    ...
    {name:'gapMm',label:'Magnetic gap (mm)',type:'number',value:12,min:.5,max:100,step:.5},
    {name:'blocksPerPeriod',label:'Blocks per period',type:'select',value:'4',options:['4','8','12','16']},
    {name:'blockWidthMm',label:'Block width x (mm)',type:'number',value:10,min:.1,max:100,step:.1},
    {name:'blockHeightMm',label:'Block height / radial thickness (mm)',type:'number',value:10,min:.1,max:100,step:.1},
    {name:'longitudinalFill',label:'Longitudinal fill factor',type:'number',value:.9,min:.5,max:.99,step:.01},
    {name:'brT',label:'Remanent induction Br (T)',type:'number',value:1.2,min:.01,max:3,step:.05},
    {name:'targetB0Enabled',label:'Calibrate Br to target B0',type:'checkbox',value:false},
    {name:'targetB0T',label:'Target B0 (T)',type:'number',value:.15,min:.001,max:20,step:.01},
    {name:'b0Definition',label:'B0 definition',type:'select',value:'Central-period peak B⊥',options:['Central-period peak B⊥','Central 3-period peak B⊥','Global peak B⊥']},
    {name:'materialMode',label:'Magnet model',type:'select',value:'Fixed remanence',options:['Fixed remanence','Linear NdFeB + relaxation']},
    {name:'muParallel',label:'μr parallel',type:'number',value:1.05,min:1,max:3,step:.01},
    {name:'muPerpendicular',label:'μr perpendicular',type:'number',value:1.05,min:1,max:3,step:.01},
    {name:'segmentation',label:'Magnet subdivision',type:'select',value:'1',options:['1','2','3']},
    {name:'relax',label:'Run RADIA relaxation',type:'checkbox',value:false},
    {name:'precision',label:'Relaxation precision (T)',type:'number',value:1e-4,min:1e-7,max:1e-2,step:1e-5},
    {name:'maxIter',label:'Relaxation max iterations',type:'number',value:1000,min:1,max:10000,step:100},
    {name:'ellipticity',label:'Ellipticity',type:'number',value:.5,min:0,max:1,step:.01},
    {name:'applePhaseDeg',label:'APPLE-II magnetic row phase (deg)',type:'number',value:90,min:-180,max:180,step:1},
    {name:'appleShiftMode',label:'APPLE-II shift mode',type:'select',value:'Antiparallel',options:['Antiparallel','Parallel']},
    {name:'errorsEnabled',label:'Enable manufacturing errors',type:'checkbox',value:false},
    {name:'fieldErrorPct',label:'Field amplitude error σ (%)',type:'number',value:1,min:0,max:25,step:.1},
    {name:'longitudinalErrorMm',label:'Longitudinal position error σ (mm)',type:'number',value:.05,min:0,max:10,step:.01},
    {name:'transverseErrorMm',label:'Transverse position error σ (mm)',type:'number',value:.05,min:0,max:10,step:.01},
    {name:'angleErrorDeg',label:'Magnetization angle error σ (deg)',type:'number',value:.5,min:0,max:10,step:.1},
    {name:'gapAsymmetryMm',label:'Gap asymmetry (mm)',type:'number',value:0,min:-10,max:10,step:.01},
    {name:'bankImbalancePct',label:'Bank strength imbalance (%)',type:'number',value:0,min:-25,max:25,step:.1},
    {name:'errorSeed',label:'Random seed',type:'number',value:12345,min:0,max:100000000,step:1},
    {name:'compareIdeal',label:'Compute ideal-vs-error comparison',type:'checkbox',value:true},
    {name:'axisSamples',label:'On-axis samples',type:'number',value:1000,min:100,max:4000,step:100},
    {name:'fieldMarginPeriods',label:'Longitudinal field margin (periods)',type:'number',value:1,min:0,max:10,step:.5},
    {name:'electronEnergyGeV',label:'Electron energy (GeV)',type:'number',value:3,min:.01,max:1000,step:.1},
    {name:'calculate2d',label:'Calculate 2D field slice',type:'checkbox',value:true},
    {name:'calculate3d',label:'Calculate sparse 3D field map',type:'checkbox',value:true},
    {name:'transverseHalfWidthMm',label:'Transverse map half-width (mm)',type:'number',value:5,min:.1,max:100,step:.5},
    {name:'geometryLimit',label:'Maximum blocks in 3D geometry viewer',type:'number',value:600,min:100,max:1200,step:100}
  ],
  'radiation-platform':[
