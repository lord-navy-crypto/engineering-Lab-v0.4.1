(() => {
  ...
  function featuredCard(s,index,total,buttonLabel){const launch=hostResolution(s),state=statuses.get(launch.host)?.state||'Not checked',disabled=busySurface===s.id?'disabled':'';return `<article class="featured-workspace-card"><div class="family-step">${index+1} / ${total} · ${esc(accessLabel(s))}</div><h4>${esc(s.label)}</h4><p>${esc(descriptions[s.id]||s.routeHint||'Engineering Lab workspace.')}</p><div class="capability-meta"><span>${esc(launch.host)}</span><span>${esc(state)}</span></div><button class="primary" data-surface-open="${esc(s.id)}" ${disabled}>${busySurface===s.id?'Preparing…':buttonLabel}</button></article>`;}
  ...
  function renderCorePlatform(){
    ...
    const rows=CORE_PLATFORM_PRIORITY.map(id=>surfaces.find(s=>s.id===id)).filter(Boolean), count=byId('corePlatformCount');
    if(count)count.textContent=`${rows.length} / ${CORE_PLATFORM_PRIORITY.length} connected`;
  ...
  function render(){
    const grid=byId('capabilityGrid'); if(!grid)return; renderUtubeFamily(); renderCorePlatform(); const needle=query.trim().toLowerCase();
